<?php
/**
 * Created by PhpStorm.
 * User: Erkin
 * Date: 17.08.2017
 * Time: 13:22
 */
return [
    'aboutus' => "Jamoa haqida",
    'categories' => "Sohalar bo'yicha savolar ",
    'blog' => "Maqolalar",
    'askquestion' => "Savol berish",
    'ordercall' => "Qo'ng'iroq buyurtma qilish",
    'orderdocument' => "Hujjat buyurtma qilish",
    'questions' => "Savollar",
    'confidence' => "Har bir qaror ishonch bilan",
    'search' => "Qidirish uchun so'zni kiriting",
    'answeredquestions' => "Javob berilgan savollar",
    'callback' => "Qayta qo'ng'iroqlar",
    'madedocuments' => "Ishlab chiqilgan hujjatlar",
    'soon' => "Tez orada",
    'callinprocess' => "\"Qo'ng'iroq buyurtma qilish\" xizmati ishlab chiqish jarayonida",
    'close' => "Yopish",
    'documentinprocess' => "\"Hujjat buyurtma qilish\" xizmati ishlab chiqish jarayonida",
    'asked' => "",
    '#questions' => "ta berilgan savollar",
    'paid' => "Pullik",
    'free' => "Bepul",
    'answers' => "javoblar",
    'price' => "qiymati",
    'sum' => "sum",
    'allquestions' => "Barcha savollar ro'yxati",
    'helpforanyquestion' => "Har qanday huquqiy masala bo'yicha yordam",
    'helpforanyquestionbody' => "bizning yuristlarimiz sizga minglab huquqiy masalalar bo'yicha tavsiya va fikrlar berishadi",
    'allcategories' => "Barcha kategoriyalar ro'yxati",
    'blogs' => "Maqolalar",
    'readblog' => "Maqolani o'qish",
    'allblogs' => "Barcha maqolalar ro'yxati",
    'consultionfrom' => "",
    'lawyers&jurists' => "ta yurist va advokatlardan maslahat",
    'ourlawyers-...' => "Bizning  jamoamiz  qonunchilikni yaxshi biladigan, yetarli amaliy tajribaga ega professional huquqshunoslardan iborat. Bizga murojaat qilib Siz o'z savolingizga asosli va to'g'ri javob olasiz",
    'feedbacks' => "ta minnatdorchiliklar",
    'allprojectlawyers' => "Barcha yuristlar ro'yxati",
    'aboutustitle' => "Dunyoning istalgan nuqtasida professional yuridik yordam ",
    'aboutusbody1' => "Malakali yuridik xizmatga ehtiyojingiz bormi? Qanday qilib yurist maslahati olishni bilmayapsizmi? Advokatlik tuzilmalariga borishga vaqtingiz yo'qmi? O'zingiz mustaqil qonunchilikni o'rganib savollarinizga javob qidiryapsiz-u, lekin javob noto'gri bo'lishi ehtimoli tufayli sizga yordam kerakmi? ",
    'aboutusbody2' => "",
    'aboutusbody3' => "",


];
